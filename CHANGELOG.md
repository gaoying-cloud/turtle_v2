...
### OOS 验证（S17 延续）
- **OOS 区间 2024-2026**: Sharpe 1.42 > IS Sharpe 1.07，ATR 过滤无过拟合
- **汇总报告更新**: 写入 OOS 对比数据到 `results/report.md`

### 实盘同步
- **`daily_signal.py`**: `compute_signals()` 新增 `atr_pct_252` 字段；`should_enter()` 新增 ATR 百分位过滤；打印输出显示 ATR 过滤原因

### 诊断框架更新
- **`run_trade_diagnostics.py`**: 参数同步至当前基线（ATR=25, Stop=8, 6品种），新增 UBR/通道位置特征

### 实验归档

| 实验 | 结论 | 状态 |
|:--|:--|:--:|
| **S17_atr_percentile** ✅ | ATR 百分位过滤定型，OOS Sharpe 1.42 | ✅ 已合并 |
| **S15_upper_band_ratio** | 10日上轨占比无预测力（AUC≈0.49） | ❌ 归档 |
| **S16_S13_adaptive_exit** | 持仓天数自适应退出停损过紧，净值大降 | ⏸️ 搁置 |
| **S18_atr_futures** | 双向多空ATR过滤 — 缺期货数据 | ⏸️ 搁置 |
| **S19_short_analysis** | 空头+EV(+44K)但组合更差(Sharpe 0.91→0.35) | ⏸️ 搁置 |
| **S20_macd_atr** | ATR过滤对MACD无效(0笔过滤)，非通用因子 | ❌ 归档 |

### 空头分析（S19）
- 强制开启做空后，空头有正期望（69笔, +44K, 胜率39.1%）
- 但组合夏普从0.91暴跌至0.35，多空争抢资金，空头仓位管理框架不成熟
- 结论：做空需先解决利润保护/仓位管理/资金分配，当前优先级不高

### ATR 过滤机制确认（S20）
- ATR 过滤对 20日突破信号有效 ✅
- ATR 过滤对 MACD 金叉信号无效（0笔被过滤）❌
- 结论：ATR 过滤**不是**通用波动率因子，它特化于"突破信号高波动期假突破"问题
...
### 半仓事件统计隔离
- **问题**: `_execute_half_exit` 将半仓记录写入 `_my_trades`，与完整交易混在一起，导致 `stop()` 统计（胜率、总盈亏、交易次数）和退化检测规则③被半仓记录污染
- **修复**: 新增 `_half_exit_events: List[dict]` 独立列表，`_execute_half_exit` / `cmd_settle_half` 写入此处；`_my_trades` 仅保留完整交易
- `stop()` 新增「利润保护减半事件」独立汇总区块
- `_trade_summary` 新增 `"half_exits"` 字段
- `daily_signal.py` 同步：`state["half_exit_events"]` 独立存储，`cmd_settle_half` 写入事件，`run()` 输出减半事件摘要

### 闭环修复实验（已回退）
- **假设**: 减半仓后禁止加仓 + 剩余半仓收紧为 5 日低点退出 → 可防止利润回吐
- **回测结果**: 总收益从 +368% 暴跌至 +225%（−143pp），Sharpe 0.93→0.78，最大回撤 16%→20%
- **根因**: 12 笔利润保护激活的头部交易占策略总利润近半。禁止加仓关闭了趋势后半段的仓位放大器；5 日线在正常回调中过早清仓。两者叠加，12 笔大赢家的利润被大幅压缩，而交易次数不变（131→131）——同一批进出场点，仅仓位规模差异
- **策略结论**: 减半仓后继续加仓不是 bug，是趋势跟踪的自然行为——超级趋势中「减半止盈 + 继续放大」是利润保护与趋势跟踪的正确共存方式。强行闭环等于自废武功
- **处置**: `_check_pyramid` guard / 5日线退出 / `should_add` guard 全部回退；仅保留 `_half_exit_events` 统计隔离

### daily_signal.py 同步
- `cmd_settle_half`：写入 `half_exit_events`，标记 `half_closed`，防重复减半
- `cmd_settle_add`：移除 `half_closed` 阻断（与策略层回退一致）
- `should_add()`：移除 `half_closed` 阻断
- `check_exit()`：恢复纯 `stop_low_10` 退出（5 日线回退）
- `run()` 输出新增当日「利润保护减半事件」摘要区块

## [V6.1-利润保护 V6.1 状态机 + 19N 阈值 + 全基线] - 2026-07-01
### 核心新增：利润保护机制（状态机 + 半仓锁定）
- **设计哲学**: 利润保护不追求精准逃顶，追求保住极端丰厚的浮盈。该赚的赚到，超极端行情后半段让利给市场。
- **激活条件**: 持仓期最高价较入场价浮盈 ≥ 19N（19 倍开仓日 ATR）后永久激活
- **激活使用 high_since_entry 而非 close**（修复 Bug #1：用 close 导致冲高回落时保护永远不触发）
- **状态机**: 新增 `protection_activated` 字段，一旦激活永久保持（修复 Bug #2：原逻辑每日重新计算，收盘回落时跳过检查）
- **减半触发**: `low ≤ max(entry_price, high_since_entry - 2×N₀)` → `_execute_half_exit()`
- **剩余清仓**: 减半后剩余半仓仍走标准 `low < stop_low_10`（严格小于）清仓
- **固定 N₀**: 整笔交易使用开仓当日的 20 日 ATR，不逐日更新
- **Position 类**: 新增 `high_since_entry` / `half_closed` / `protection_activated` 字段
- **TurtlePositions**: 新增 `reduce_shares()` 方法（部分减仓不关闭持仓）
- **`daily_signal.py`**: 同步 `should_exit()` → `check_exit()`，支持 half/full 退出类型

### 基线绩效 (19N, ATR=20, B=20, S=10, α=0.05)
- **最终净值**: ¥468,764（初始 ¥100,000），总收益率 +368.76%，年化 13.38%
- **夏普**: 0.93（四基准最高），**最大回撤**: 16.12%（四基准最低）
- **交易**: 131 笔，胜率 48.85%，**盈亏比 2.91**
- **利润保护**: 12 次激活，合计锁定约 ¥491k
- **基准对比 B4 全面胜出**: 夏普 0.93 ≥ B1 0.71 / B2 0.66 / B3 0.41

### 压力测试修复
- **Bug: 连续止损暂停逻辑倒挂** — 原代码要求"至少触发 1 次暂停"才通过，修复为"≤1 次暂停"（策略越稳健越通过）
- **Bug: Sharpe=None** — 短区间场景 Sharpe 无法计算，改为显示 N/A(区间过短)
- **Bug: 国债依赖** — stress_test 加载 ALL_SYMBOLS 含 511010.SH 导致数据对齐失败，改为 SIX_SYMBOLS
- **vaR 显示** — 原始小数改为百分比+两位小数
- **通过率**: 修复后 14/15（原 11/15）

### 报告可读性改进
- **基准对比 B1-B4**: 表格加入中文策略名称（如 "B1 买入等权持有"）
- **压力测试摘要表**: 新增总收益%、交易数、备注列，显示未通过检查项
