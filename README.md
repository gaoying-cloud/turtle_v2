...
<!-- STATUS_START -->
当前阶段: S28 Phase 4 | 测试: 270 passed | 6品种 | **双策略体系** | 海龟17.2% / N字18.5%
<!-- STATUS_END -->
...
│   └── n_structure.py        #   N字结构策略（纯 pandas，S22定型）
├── scripts/
│   ├── run_backtest.py       #   单次回测
│   ├── run_n_structure.py    #   N字结构回测 (--portfolio 组合模式)
│   ├── run_comparison.py     #   基准对比 B1-B4
│   ├── run_stress_test.py    #   压力测试
│   ├── run_grid_search.py    #   网格搜索 + Stage-2 权重搜索
│   ├── gen_report.py         #   综合报告
│   ├── daily_signal.py       #   海龟日线信号
│   ├── daily_signal_n_structure.py  # N字结构日线信号
│   ├── scan_n_structure.py   #   N字参数扫描 (--robustness 稳健性)
│   └── ...
├── tests/                    # 270 passed (含 test_n_structure.py 25个)
