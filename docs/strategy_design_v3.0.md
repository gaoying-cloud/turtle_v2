...
| ② | **上市年限** | 覆盖至少一轮完整牛熊 | ≥ 2018 年（≥7年） | 人工 |
| ③ | **流动性** | 日均成交额 + 买卖价差 | > 2 亿 + < 5bp | 人工 |
| ④ | **品种独立盈亏能力** | 单品种海龟独立回测 | 盈亏比 ≥ 1.0, CAGR > 0 | `scripts/run_backtest.py` (1品种) |
| ⑤ | **低相关性** | 与已有品种的60日滚动相关系数 | ≤ 0.5 | `scripts/run_correlation_monitor.py` |
| ⑥ | **信号源独立性** | 优先填充"空白象限"非已有象限堆叠 | 参见下表 | — |
| ⑦ | **交易规则兼容** | T+0品种优先 | T+1 ≤ 组合总数 50% | — |
| ⑧ | **组合边际贡献** | N+1回测对比 | 夏普不降、回撤不恶化 > 2% | `scripts/run_backtest.py` |
...
核心原则：**任一 REJECT 即淘汰**，不进入后续检查；WARN 不阻断但记录。
...
| 产物 | 路径 | 说明 |
|:--|:--|:--|
| 筛查报告 | `results/screening_report.json` | 完整结构，含每个候选的 6 道检查明细 |
...
| **S10** | 品种筛选脚本 | 1天 | `scripts/screen_candidates.py` + `hurst_exponent()` | 📋 施工图就绪
