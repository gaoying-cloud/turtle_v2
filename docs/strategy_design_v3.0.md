...
version: "3.8"
date: "2026-06-16"
based_on: "V3.7 (2026-06-16)"
---

# 跨市场ETF海龟组合策略 — 设计文件 V3.8

**V3.8 变更**：
- P0-2 移动止损修复回测验证：绩效从 +2.27% 降至 -1.53%
- **退出规则重构（方案A）**：删除 2N 追尾止损（`close ≤ trail_high - 2N`），仅保留经典 10 日低点退出（`low ≤ stop_low_10`）
- `_update_trailing_stop` 不再调用，`calc_trailing_stop` 导入移除
- 对应更新 §4.2、§5.7.2、§6.1 描述
- 检测报告：`docs/analysis/moving_stop_fix_comparison.md`
