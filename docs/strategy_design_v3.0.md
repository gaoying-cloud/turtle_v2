...
version: "6.2"
date: "2026-07-01"
based_on: "V6.1 (2026-07-01)"
---

# 跨市场ETF海龟组合策略 — 设计文件 V6.2

**V6.2 变更**：
- 半仓事件统计隔离：新增 `_half_exit_events` 独立列表，`_my_trades` 仅保留完整交易，消除 `stop()` 统计及退化检测的污染
- 闭环修复实验（已回退）：减半仓后禁止加仓 + 5日线退出在回测中导致总收益 −143pp（+368%→+225%），已全部回退。结论：减半后继续加仓是趋势跟踪的正常行为，非逻辑漏洞
